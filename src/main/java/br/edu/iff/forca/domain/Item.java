...
public class Item extends ObjetoDominioImpl {
    ...
    private Item(long id, Palavra palavra, int[] posicoesDescobertas, String palavraArriscada) {
        ...
        // ao reconstituir um item que ainda não foi arriscado a palavraArriscada vem null entao nao pode ter validacao de null
    ...
    public int calcularPontosLetrasEncobertas(int valorPorLetraEncoberta) {
        if (valorPorLetraEncoberta < 0)
            throw new IllegalArgumentException("O valor por letra encoberta nao pode ser negativo");
    ...
    protected void arriscar(String palavra) {
        if (palavraArriscada != null)
            throw new IllegalStateException("Voce so pode arriscar uma vez!");
